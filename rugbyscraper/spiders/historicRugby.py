...
class HistoricRugbyResultsSpider(Spider):
    ...
    def __init__(self):
        self.missed_urls = set()
        dispatcher.connect(self.spider_closed, signals.spider_closed)
        super(HistoricRugbyResultsSpider, self).__init__()
    ...
    def scrape_result_iframe(self, response):
        try:
            ...
            try:
                gmt_ = ' ' + re.findall(
                    r'(\d{2}:\d{2})\sGMT', title_info, re.I)[0]
            except:
                gmt_ = ''
            ...
            try:
                ...
                try:
                    home, item['home_final'] = title.split(
                        '-')[0].strip().split()
                except:
                    splt = title.rsplit('-', 1)[0]
                    home = self._get_char(splt).strip()
                    item['home_final'] = self._get_num(splt)
            try:
    ...
    @staticmethod
    def _get_char(x):
        return ''.join(ele for ele in x if not ele.isdigit())
